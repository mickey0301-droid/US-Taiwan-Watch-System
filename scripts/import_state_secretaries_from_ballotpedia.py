...
SOURCE_URL = "https://ballotpedia.org/Secretary_of_State_(state_executive_office)"
USER_AGENT = "Mozilla/5.0 (compatible; UTWBot/1.0; +https://github.com/mickey0301-droid/US-Taiwan-Watch-System)"
PARSER_IDENTITY = "ballotpedia_state_secretary_v1"
...
def _extract_state_and_role(office_label: str) -> tuple[str, str] | None:
    text = _clean_text(office_label)

    m = re.match(r"^(.+?) Secretary of State$", text, flags=re.I)
    if m:
        state = _clean_text(m.group(1))
        if state in US_STATES:
            return state, "Secretary of State"

    m = re.match(r"^(.+?) Secretary of the Commonwealth$", text, flags=re.I)
    if m:
        state = _clean_text(m.group(1))
        if state in US_STATES:
            return state, "Secretary of the Commonwealth"
...
def _extract_records(html: str) -> list[dict[str, str]]:
    ...
    for row in target_table.select("tr"):
        ...
        parsed = _extract_state_and_role(office_label)
        if not parsed:
            continue
        state_name, role_title = parsed
    ...
    with session_scope() as session:
        ...
        for item in records:
            ...
            if item["role_title"] == "Secretary of State":
                service.ensure_alias(person.id, f"Secretary {item['person_name']}", item["source_url"], "media")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Import current state secretaries from Ballotpedia.")
