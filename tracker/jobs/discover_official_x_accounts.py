...
def run_discover_official_x_accounts(limit: int | None = None) -> dict:
    with session_scope() as session:
        service = SocialProfileBackfillService(session)
        sync_run = SyncRun(
            job_name="discover_official_x_accounts",
            job_type="profile_enrichment",
            source_name="official_websites",
        )
        session.add(sync_run)
        session.flush()

        result = service.backfill_x_profiles(limit=limit, current_federal_only=True)
        ...
        sync_run.meta = {
            ...
            "limit": limit,
            "mode": "current_federal_official_only",
        }
        return {
