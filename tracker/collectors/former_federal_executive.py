...
class FormerFederalExecutiveCollector(BaseCollector):
    collector_name = "former_federal_executive"
    source_name = "Biden White House Cabinet Archive"
    source_url = "https://bidenwhitehouse.archives.gov/administration/cabinet/"
    parser_identity = "biden_whitehouse_cabinet_archive_v1"
    ...
    def fetch(self) -> str:
        ...
        if self.settings.snapshot_raw_responses:
            ...
            (snapshot_dir / f"former_federal_executive_{datetime.utcnow():%Y%m%d%H%M%S}.html").write_text(response.text, encoding="utf-8")
    ...
    def parse(self, payload: str) -> list[dict[str, Any]]:
        ...
        for card in soup.select(".persongrid__item, .persongrid__item__inner"):
            name_heading = card.find("h3")
            role_heading = card.find("h4")
            if not name_heading or not role_heading:
                continue
            full_name = " ".join(name_heading.get_text(" ", strip=True).split())
            role_title = " ".join(role_heading.get_text(" ", strip=True).split())
            ...
            parsed.append(
                {
                    "person": {
                        ...
                        "verification_status": "archive_official",
                        "raw_payload": {"office_title": role_title, "archive_source_page": self.source_url},
                    },
    ...
    def sync(self) -> CollectorRunResult:
        ...
        with session_scope() as session:
            ...
            try:
                ...
                former_term = roster_service.presidential_term_roster(46)
                ...
                logger.exception("Former federal executive collector failed.")
                result.errors.append(str(exc))
