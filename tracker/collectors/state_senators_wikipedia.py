...
from tracker.utils.state_sources import STATE_SENATE_WIKIPEDIA_SOURCES, merge_sources
...
class StateSenatorsWikipediaCollector(BaseCollector):
    collector_name = "state_senators_wikipedia"
    source_name = "Wikipedia state senate rosters"
    parser_identity = "wikipedia_state_senators_v1"

    def __init__(self) -> None:
        self.settings = get_settings()

    def fetch(self) -> list[dict[str, Any]]:
        configured = get_source_registry().get("state_senate_wikipedia_sources", [])
        return merge_sources(configured, STATE_SENATE_WIKIPEDIA_SOURCES)
    ...
    def parse(self, payload: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        for source in payload:
            try:
                ...
                logger.exception("State senate wikipedia parse failed for %s", source.get("state"))
                records.append(
                    {
                        "_error": {
    ...
    def sync(self) -> CollectorRunResult:
        ...
        with session_scope() as session:
            ...
            try:
                ...
                logger.exception("State senators wikipedia collector failed.")
                result.errors.append(str(exc))
    ...
    def _parse_state_page(self, source: dict[str, Any], html: str) -> list[dict[str, Any]]:
        ...
        for row in table.select("tr"):
            ...
            if not cells or not row.find("td"):
                continue
            member = self._parse_member_row(row, state, page_url)
            if member:
    ...
    def _find_members_table(self, soup: BeautifulSoup) -> Tag | None:
        for table in soup.select("table.wikitable"):
            ...
            if "district" not in header_text:
                continue
            if any(token in header_text for token in ["senator", "name", "member"]):
                return table
    ...
    def _parse_member_row(self, row: Tag, state: str, page_url: str) -> dict[str, Any] | None:
        ...
        office_name = f"{state} State Senate"
        role_title = "State Senator"
        aliases = [f"Sen. {full_name}", f"{state} state senator {full_name}"]
        return {
            "person": {
                "full_name": full_name,
                ...
                "raw_payload": {
                    ...
                    "subdepartment_name": "State Senate",
                    "unit_name": district,
                },
            },
